...
class CustomedPipeline():
    ...
    def load_data(self, file_path, o_batch_size, i_batch_size):
        ...
        tokenized = self.tokenizer.apply_chat_template(messages,
                                               tokenize=True,
                                               padding=True,
                                               truncation=True,
                                               return_tensors="pt",
        ...
        batches = self.batchify(tokenized, o_batch_size)
        ...
        for outer_batch in outer_batches:
            inner_batch = []
            inner_batch_m = []
            for i, token in enumerate(outer_batch):
                inner_batch.append(token[i].tokens)
                inner_batch_m.append(token[i].attention_mask)
                print(token[i].ids)
    ...
    def forward(self, max_new_tokens):
        ...
        for batch in zip(self.input_ids, self.attention_mask):
            ...
            print('out 길이',len(outs))
            for out in outs:
                result.append(out)
            end = time.time()
            
            print('batch load and inference time ', (end - st)/ (self.o_batch_size*self.i_batch_size))
            times += end - st
            cnt += 1
            break
            if cnt % 5 == 0:
