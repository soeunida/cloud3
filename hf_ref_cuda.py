...
class Phi3Attention(nn.Module):
    ...
    def forward(
        ...
        offsets = torch.zeros(bsz).to(device=self.config.device)
        ...
        query_states = apply_rotary_emb(
                query_states, cos, sin, seqlen_offsets=offsets, interleaved=True
            )
        key_states = apply_rotary_emb(
            key_states, cos, sin, seqlen_offsets=offsets, interleaved=True
        )
...
class Phi3FlashAttention2(nn.Module):
    ...
    def forward(
        ...
        offsets = torch.zeros(bsz).to(dtype=torch.int32, device=self.config.device)
        cos = torch.cos(angle).to(dtype=torch.float32)
        sin = torch.sin(angle).to(dtype=torch.float32)
